...
const VALID_UNITS=new Set(["piece","meter","kg","litre","box"]);
...
function normalizeProduct(id,data){const images=Array.isArray(data.gallery)&&data.gallery.length?data.gallery:Array.isArray(data.images)&&data.images.length?data.images:data.image?[data.image]:[DEFAULT_IMAGE];const sizes=normalizeSizes(data);const unit=VALID_UNITS.has(data.unit)?data.unit:"piece";return{...data,id:String(data.id||id).trim(),image:images[0]||DEFAULT_IMAGE,gallery:images.length?images:[DEFAULT_IMAGE],name:data.name||localizedValue(data.nameEn,data.nameHi),description:data.description||localizedValue(data.descriptionEn,data.descriptionHi),features:data.features||{hi:[],en:[]},specifications:data.specifications||{hi:{},en:{}},sizes,mrp:Number(data.mrp)||0,unit,featured:Boolean(data.featured),status:data.status==='hidden'?'hidden':'active'}}
