...
async function requestCatalogue(){const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),10000);try{const response=await fetch(`${CATALOGUE_URL}?v=${Date.now()}`,{cache:"no-store",signal:controller.signal});if(!response.ok)throw new Error(`Catalogue request failed: ${response.status}`);const data=await response.json();if(!Array.isArray(data))throw new Error("Catalogue data is invalid.");return data.map((item,index)=>normalizeProduct(item?.id||`P${index+1}`,item||{}))}finally{clearTimeout(timeout)}}
