  ...
  try {
    ...
    const renderRelated = () => {
      ...
      document.getElementById("related-products").innerHTML = items.length
        ? items.map((item) => `<article class="product-card"><img src="${escapeHTML(item.image)}" loading="lazy" alt="${escapeHTML(item.name[language()])}"><div class="product-card-content"><h3>${escapeHTML(item.name[language()])}</h3>${formatMrp(item.mrp)?`<div class="product-mrp"><span>${text.mrp}</span><strong>${formatMrp(item.mrp)}</strong></div>`:""}<p>${escapeHTML(item.description[language()])}</p><a class="primary-btn" href="product.html?id=${encodeURIComponent(item.id)}">${text.details}</a></div></article>`).join("")
    ...
    const render = () => {
      ...
      const mrp = formatMrp(product.mrp);
