  ...
  try{
    ...
    const renderRelated=()=>{
      ...
      relatedTarget.innerHTML=items.length?items.map(item=>{
        ...
        return `<article class="product-card"><img src="${escapeHTML(image)}" loading="lazy" alt="${escapeHTML(item.name?.[language()]||item.name?.en||item.name?.hi||item.id)}"><div class="product-card-content"><h3>${escapeHTML(item.name?.[language()]||item.name?.en||item.name?.hi||item.id)}</h3>${display?`<div class="product-mrp"><span>${text.mrp}</span><strong>${label}</strong></div>`:""}<p>${escapeHTML(item.description?.[language()]||item.description?.en||item.description?.hi||"")}</p><a class="primary-btn" href="product.html?id=${encodeURIComponent(item.id)}">${text.details}</a></div></article>`;
    ...
    const render=()=>{
      ...
      target.innerHTML=`<div class="product-detail-layout"><section class="product-gallery" aria-label="${escapeHTML(name)} gallery"><img class="product-main-image" id="main-product-image" src="${escapeHTML(mainImage)}" alt="${escapeHTML(name)}" tabindex="0" role="button" aria-label="Open ${escapeHTML(name)} image"><div class="thumbnail-list">${gallery.filter(Boolean).map((image,index)=>`<button type="button" class="thumbnail ${index===selectedImage?"active":""}" data-image-index="${index}" aria-label="${escapeHTML(name)} image ${index+1}"><img src="${escapeHTML(safeImage(image))}" alt="" loading="lazy"></button>`).join("")}</div></section><section class="product-info-card"><span class="product-meta">${escapeHTML(product.brand||"")}</span><h1>${escapeHTML(name)}</h1>${mrp?`<div class="product-detail-mrp"><span>${text.mrpPerPiece}</span><strong>${mrp}</strong></div>`:""}<p>${escapeHTML(product.description?.[language()]||product.description?.en||product.description?.hi||"")}</p>${sizes.length?`<h2 class="detail-heading">${text.selectSize}</h2><div class="size-options" role="group" aria-label="${text.selectSize}">${sizes.map((size,index)=>`<button type="button" class="size-option ${index===selectedSizeIndex?"active":""}" data-size-index="${index}">${escapeHTML(sizeName(size))}</button>`).join("")}</div>`:""}<h2 class="detail-heading">${text.features}</h2><ul class="feature-list">${featureValues.map(feature=>`<li>${escapeHTML(feature)}</li>`).join("")}</ul><a class="secondary-btn detail-enquiry" id="product-enquiry" target="_blank" rel="noopener" href="#">${text.enquire}</a></section></div>`;
